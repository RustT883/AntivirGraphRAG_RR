...
def stratified_dirty_split_data_by_relation_type(data, num_unseen_rel_types, max_test_size):
    '''
    Stratified version of dirty_split_data_by_relation_type.
    Ensures test set contains a balanced sample of each target relation type,
    while still allowing train/test relation overlap and respecting max_test_size.
    '''
    logger.info("Stratified dirty splitting data...")
    ...
    while not correct_num_unseen_relations_achieved:
        ...
        # Group data items by their relation types (only for test relations)
        reltype_to_items = defaultdict(list)
        other_items = []
        
        for item in data:
            item_relations = {r["relation_text"] for r in item['relations']}
            overlapping_test_rels = item_relations & test_relation_types
            
            if overlapping_test_rels:
                # Assign to the least represented test relation to balance distribution
                least_common_rel = min(overlapping_test_rels, 
                                     key=lambda r: len(reltype_to_items[r]))
                reltype_to_items[least_common_rel].append(item)
            else:
                other_items.append(item)

        # Build test set by taking up to max_test_size // num_unseen_rel_types per relation
        test_data = []
        samples_per_rel = max(1, max_test_size // num_unseen_rel_types)
        
        for rel in test_relation_types:
            test_data.extend(reltype_to_items[rel][:samples_per_rel])
        
        # If we didn't fill test_size, distribute remaining slots
        if len(test_data) < max_test_size:
            remaining_slots = max_test_size - len(test_data)
            all_remaining = [item for rel in test_relation_types 
                           for item in reltype_to_items[rel][samples_per_rel:]]
            random.shuffle(all_remaining)
            test_data.extend(all_remaining[:remaining_slots])

        # Everything else goes to train
        train_data = other_items + [item 
                                   for rel in test_relation_types 
                                   for item in reltype_to_items[rel] 
                                   if item not in test_data]
        
        # Check termination conditions
        actual_unseen = len(get_unique_relations(test_data))
        if (actual_unseen == original_num_unseen_rel_types or 
            len(test_data) >= max_test_size):
            correct_num_unseen_relations_achieved = True
        else:
            # Adjust num_unseen_rel_types as in original
            num_unseen_rel_types = (num_unseen_rel_types + 1 
                                   if num_unseen_rel_types < original_num_unseen_rel_types * 2 
                                   else num_unseen_rel_types)
...
def train(model, optimizer, train_data, config, train_rel_types, eval_rel_types, eval_data=None, 
          ...
          wandb_log=False, wandb_sweep=False, warmup_ratio=0.1, train_batch_size=1, device='cuda', use_amp=False):
...
# training_loop_patch.py
# Add this to your train.py file or modify the existing training loop
...
def enhanced_training_step(model, batch, optimizer, scaler, tb_writer, step, config):
    """
    Enhanced training step with memory management and logging
    """
    model.train()
    ...
    with autocast(enabled=True):  # Always use mixed precision for memory efficiency
        # Forward pass
        outputs = model(batch)
        loss = outputs.loss if hasattr(outputs, 'loss') else outputs
        
        # Scale loss for gradient accumulation
        if hasattr(config, 'gradient_accumulation') and config.gradient_accumulation > 1:
    ...
    if (step + 1) % getattr(config, 'gradient_accumulation', 1) == 0:
        # Gradient clipping
    ...
    if tb_writer is not None:
        ...
        tb_writer.add_scalar('Training/Gradient_Norm', total_norm, step)
        
        # Memory logging every 100 steps
        if step % 100 == 0:
            if torch.cuda.is_available():
                allocated = torch.cuda.memory_allocated() / 1024**3
                cached = torch.cuda.memory_reserved() / 1024**3
                tb_writer.add_scalar('Memory/GPU_Allocated_GB', allocated, step)
                tb_writer.add_scalar('Memory/GPU_Cached_GB', cached, step)
    
    return loss.item()

def enhanced_evaluation_step(model, eval_data, eval_rel_types, tb_writer, step, config):
    """
    ...
    with torch.no_grad():
        # Run your existing evaluation logic here
        # This is a placeholder - you'll need to adapt to your existing eval code
        
        total_eval_loss = 0
        num_batches = 0
        
        for batch in eval_data:  # Assuming eval_data is iterable
            with autocast(enabled=True):
                outputs = model(batch)
                eval_loss = outputs.loss if hasattr(outputs, 'loss') else outputs
                total_eval_loss += eval_loss.item()
                num_batches += 1
        
        avg_eval_loss = total_eval_loss / num_batches if num_batches > 0 else 0
        
        # Log evaluation metrics
        if tb_writer is not None:
            tb_writer.add_scalar('Evaluation/Loss', avg_eval_loss, step)
            
            # Add more metrics as they become available from your evaluation
            # tb_writer.add_scalar('Evaluation/F1_Score', f1_score, step)
            # tb_writer.add_scalar('Evaluation/Precision', precision, step)
            # tb_writer.add_scalar('Evaluation/Recall', recall, step)
...
class MemoryEfficientDataLoader:
    ...
    def __iter__(self):
        ...
        for i in range(0, len(indices), self.batch_size):
            ...
            # Yield batch and clean up references
            yield batch
...
# Improved configuration for memory efficiency
def get_memory_efficient_config():
    """
    Returns optimized configuration for 6GB GPU
    """
    return {
        'train_batch_size': 1,  # Very small batch size
        'eval_batch_size': 1,
        'gradient_accumulation': 8,  # Effective batch size of 8
        'max_len': 256,  # Reduced sequence length
        'num_steps': 50000,  # Reduced for testing
        'eval_every': 1000,
        'lr_encoder': 1e-5,  # Conservative learning rate
        'lr_others': 5e-5,
...
def memory_efficient_training_loop(model, train_data, eval_data, config, tb_writer, logger):
    """
    Memory-efficient training loop with comprehensive logging
    """
    ...
    for epoch in range(config.get('num_epochs', 100)):
        for batch in train_loader:
            # Training step
            loss = enhanced_training_step(
                model, batch, optimizer, scaler, tb_writer, step, config
            )
            ...
            if step % config['eval_every'] == 0 and step > 0:
                eval_results = enhanced_evaluation_step(
                    model, eval_data, eval_rel_types, tb_writer, step, config
                )
